...
pSigma = -4*pi*z0*100*sigma+4*pi*2*sigma.^2;
...
% zeroPhase = findZeroPhase(zDataInter,intensity,phase,0);
...
% 
% function zeroPhase = findZeroPhase(zData,intensity,phaseData,zeroPoint)
% diff = 0.01;
% [zDataMax,zDataMaxindex]= max(intensity);
% ampMaxZ = zData(zDataMaxindex);
% %limit the range of zData,about (zp-lambda/8,zp+lambda/8)
% LimZData = zData(zData>(ampMaxZ-(1/1.45)/8)&zData<(ampMaxZ+(1/1.45)/8));
% phaseData = phaseData(zData>(ampMaxZ-(1/1.45)/8)&zData<(ampMaxZ+(1/1.45)/8));
% % phaseData = phaseData()
